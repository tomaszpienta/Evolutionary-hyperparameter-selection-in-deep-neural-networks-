...
def extract_grids(dataset_path: str, ground_truth_path: str, window_size: Tuple,
    ...
    x = 0
    y = 0
    for i in range(gt.shape[0] * gt.shape[1]):
        labels_names.add(gt[y, x])
        x = x + 1
        if x == gt.shape[1]:
            x = 0
            y += 1
    ...
    for i in range(gt.shape[0] * gt.shape[1]):
        ...
        if gt[y, x] not in labels_dictionary:
            labels_dictionary[gt[y, x]] = unused_label
            unused_label += 1
        gt[y, x] = labels_dictionary[gt[y, x]]
        labels_values.add(gt[y, x])
        x = x + 1
        if x == gt.shape[1]:
            x = 0
            y += 1
